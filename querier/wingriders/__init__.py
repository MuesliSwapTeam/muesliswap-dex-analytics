...
def determine_sell_amount(txo: dict, sell_token: Token):
    ...
    if sell_token.policy_id == "" and sell_token.name == "":
        ...
        assert amount > 0, "Invalid wingriders order: not enough lvl attached after fees"
        ...
        assert amount > 0, "Invalid wingriders order: specified sell token wasn't attached"
        if attached_lvl != 4_000_000:
            _LOGGER.warning(f"Unexpected Wingriders attached lvl {attached_lvl} != 4_000_000")
        return Asset(token=sell_token, amount=amount)
...
def parse_swap_datum(txo: dict, datum: Datum) -> OrderValues:
    ...
    fields = datum["fields"]
    assert isinstance(fields, list) and len(fields) == 2, "expected 2 fields for wingriders"

    properties1, properties2 = fields
    assert len(properties1["fields"]) == 4, "properties 1 length must be 4 but is %d" % len(properties1["fields"])
    trader_obj, beneficiary_pkh, _, _ = properties1["fields"]
    tokens = get_assets(properties1)

    assert len(properties2["fields"]) == 2, "properties 2 length must be 2 but is %d" % len(properties2["fields"])
    ...
    rcvd_token = Token(
        policy_id=parse_bytes(rcvd_policy_id, "policy-id"), name=parse_bytes(rcvd_token_name, "tokenname")
    )
    rcvd_asset = Asset(amount=buy_amount, token=rcvd_token)

    place_policy_id, place_token_name = tokens[swap_dir]
    place_token = Token(
        policy_id=parse_bytes(place_policy_id, "policy-id"), name=parse_bytes(place_token_name, "tokenname")
    )
...
def get_assets(props1: Datum):
    ...
    assert isinstance(fields, list) and len(fields) == 4, "expected 4 fields for sundae swap #1"
    address_obj, pubkeyhash, unix_ts, token_constr = fields
    tokens_list = token_constr["fields"]
    assert isinstance(tokens_list, list) and len(tokens_list) == 2, "expected 2 tokens in sundae tx"
