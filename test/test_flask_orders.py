...
def test_orderbook():
    ...
    # from ada
    endpoint_url = f'/orderbook?from-token={base_token}&to-token={quote_token}'
    response = client.get(endpoint_url)
    assert response.status_code == 200
    # to ada
    endpoint_url = f'/orderbook?from-token={quote_token}&to-token={base_token}'
    response = client.get(endpoint_url)
    assert response.status_code == 200
    # twice the same token
    endpoint_url = f'/orderbook?from-token={quote_token}&to-token={quote_token}'
    response = client.get(endpoint_url)
    assert response.status_code == 400
    # only one token
    endpoint_url = f'/orderbook?from-token={quote_token}'
    response = client.get(endpoint_url)
    assert response.status_code == 400
